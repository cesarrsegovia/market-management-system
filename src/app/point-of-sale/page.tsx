...
export default function PointOfSalePage() {
    ...
    const addToCart = (product: Product) => {
        ...
        if (existingCartItemIndex > -1) {
            // 3. Producto existe: Incrementar cantidad si el stock lo permite
            ...
            if (existingItem.quantityInCart < availableStock) {
                ...
                setError(`No puedes añadir más ${product.name}, stock máximo alcanzado.`);
            }
        } else {
            // 4. Producto no existe: Añadirlo con cantidad 1
            const newCartItem: CartItem = {
                ...product,
                quantityInCart: 1,
            };
            setCart([...cart, newCartItem]); // Añadir el nuevo item al estado del carrito
        }
    };
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            await axios.post('/api/sales', saleData, { withCredentials: true });
    ...
    return (
        <div className="container mx-auto p-4 grid grid-cols-3 gap-4 h-screen">
            ...
            <div className="col-span-2 bg-white p-4 rounded shadow flex flex-col">
                ...
                {error && <p className="text-red-500 mb-2">{error}</p>} {/* Mostrar errores aquí también */}
                <div className="flex-grow overflow-y-auto border-b mb-4">
                    {cart.length === 0 && <p className="text-gray-700">El carrito está vacío.</p>}
                    <ul>
                        {cart.map((item) => (
                            <li key={item.id} className="py-2 flex justify-between items-center text-gray-800">
                                {/* Mostrar detalles del item */}
                                <span>{item.quantityInCart} x {item.name} (${(item.price * item.quantityInCart).toFixed(2)})</span>
                                {/* TODO: Añadir botones para ajustar cantidad o eliminar item */}
                            </li>
                        ))}
                    </ul>
                </div>
                <div className="text-right">
                    ...
                    <p className="text-xl font-bold mb-4 text-gray-800">
                        Total: ${cart.reduce((sum, item) => sum + item.price * item.quantityInCart, 0).toFixed(2)}
                    </p>
                    <button
